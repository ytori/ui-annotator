...
const componentCache = new WeakMap<
	object,
	{ components: UIComponentSuggestion[]; error?: string }
>();

/**
 * Load components from a stored source using the appropriate loader
 */
function loadSourceComponents(source: StoredSourceData): {
	components: UIComponentSuggestion[];
	error?: string;
} {
	// Check cache first
	...
	if (manifest && typeof manifest === "object") {
		const cached = componentCache.get(manifest);
		if (cached) return cached;
	}

	// Try stored loaderId first, then fall back to auto-detection
	let loader = getLoaderById(source.loaderId);
	if (!loader) {
		loader = findLoader(source.manifest);
	}

	if (!loader) {
		const error = `No loader found for format (loaderId: ${source.loaderId})`;
		console.warn(`[ComponentCatalog] ${error}`);
		const result = { components: [], error };
		if (manifest && typeof manifest === "object") {
	...
	try {
		...
		const result = { components: [] as UIComponentSuggestion[], error };
		if (manifest && typeof manifest === "object") {
			componentCache.set(manifest, result);
		}
		return result;
	...
	persist(
		(set, get) => ({
			...
			updateSourceName: (sourceId, name) => {
				if (!name.trim() || sourceId === BUILTIN_ID) return;
				set((state) => ({
					importedSources: state.importedSources.map((s) =>
