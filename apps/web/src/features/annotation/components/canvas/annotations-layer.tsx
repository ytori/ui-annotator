...
/**
 * Apply snap adjustments to edges, keeping opposite edge fixed.
 */
function applyEdgeSnaps(
  ...
  // Horizontal snaps (mutually exclusive)
  if (snapLeft !== null) {
    left = snapLeft;
  ...
  // Vertical snaps (mutually exclusive)
  if (snapTop !== null) {
    top = snapTop;
...
export function AnnotationsLayer({
  ...
  const handleElementTransform = useCallback(
    (e: Konva.KonvaEventObject<Event>, id: ElementId) => {
      ...
      // Apply snaps and calculate new dimensions
      const snapped = applyEdgeSnaps(
        edges,
        snapLeft,
        snapRight,
        snapTop,
      ...
      const newW = snapped.right - snapped.left;
      const newH = snapped.bottom - snapped.top;

      group.x(snapped.left);
      group.y(snapped.top);
